...
defmodule QcommerceWeb.Live.Components.CartPanel do
  ...
  def cart_panel(assigns) do
    ...
    <%= if @show_cart do %>
      <%!-- Backdrop --%>
      <div style="position:fixed;inset:0;background:rgba(0,0,0,.4);z-index:400"
           phx-click="close_cart"></div>
      ...
      <aside style="position:fixed;top:0;right:0;bottom:0;width:100%;max-width:400px;background:#fff;box-shadow:-4px 0 32px rgba(0,0,0,.15);display:flex;flex-direction:column;z-index:401;font-family:system-ui,sans-serif">
        ...
        <div style="display:flex;align-items:center;gap:8px;padding:14px 16px;border-bottom:1px solid #e5e7eb">
          <%= if @show_share_panel do %>
            <button phx-click="hide_share_panel"
              style="background:#f3f4f6;border:none;border-radius:8px;width:28px;height:28px;cursor:pointer;font-size:16px;display:flex;align-items:center;justify-content:center">←</button>
            <span style="font-size:15px;font-weight:700;color:#111827">Share cart</span>
            <span style="background:#dbeafe;color:#1e40af;border-radius:100px;padding:2px 8px;font-size:11px;font-weight:600;margin-left:4px"><%= @cart_count %> items</span>
          <% else %>
            <span style="font-size:15px;font-weight:700;color:#111827">🛒 Your cart</span>
            <%= if @cart_count > 0 do %>
              <span style="background:#dcfce7;color:#166534;border-radius:100px;padding:2px 8px;font-size:11px;font-weight:600"><%= @cart_count %> items</span>
            <% end %>
          <% end %>
  ...
  defp share_panel(assigns) do
    ...
    <div style="flex:1;overflow-y:auto;padding:14px 16px">
      ...
      <div style="padding:8px 12px;background:#fef3c7;border:1px solid #fde68a;border-radius:8px;font-size:12px;color:#92400e;display:flex;gap:6px;margin-bottom:14px">
        <span>ℹ️</span>
        <span>Anyone with this link can view and add these items to their cart — no account needed.</span>
      </div>

      <%!-- Items preview --%>
      <div style="font-size:11px;font-weight:600;color:#9ca3af;text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px">Items in share</div>
      <%= for {_pid, item} <- @cart_items do %>
        <div style="display:flex;align-items:center;gap:10px;padding:8px 10px;background:#f9fafb;border-radius:8px;margin-bottom:6px;border:1px solid #e5e7eb">
          ...
          <span style="background:#dcfce7;color:#166534;border-radius:100px;padding:2px 8px;font-size:11px;font-weight:600">×<%= item.qty %></span>
        </div>
      <% end %>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:14px">
        ...
        <button phx-click="show_share_qr"
      ...
      <%!-- Copy link --%>
      <div style="font-size:11px;font-weight:600;color:#9ca3af;text-transform:uppercase;letter-spacing:.5px;margin-bottom:6px">Or copy link</div>
      ...
      <div style="margin-top:12px;padding:10px 12px;background:#f9fafb;border-radius:8px;border:1px solid #e5e7eb">
        ...
        <div style="font-size:11px;color:#9ca3af;margin-top:2px">Recipients can add these items within this window</div>
      </div>

      <%!-- QR code (toggled) --%>
      <%= if @show_share_qr do %>
        <div style="margin-top:12px;text-align:center;padding:16px;background:#f9fafb;border-radius:12px;border:1px solid #e5e7eb">
          <div style="font-size:13px;font-weight:600;color:#111827;margin-bottom:10px">Scan to open cart</div>
          <img src={"https://api.qrserver.com/v1/create-qr-code/?size=160&color=0c831f&data=#{URI.encode(@share_url)}"}
               style="border:3px solid #16a34a;border-radius:12px;padding:6px;display:block;margin:0 auto" />
          <div style="font-size:11px;color:#9ca3af;margin-top:8px">Works on any device — no app needed</div>
        </div>
      <% end %>
    </div>
      ...
      <% else %>
        ...
        <%= if savings_amount(@cart_items) > 0 do %>
          <div style="display:flex;align-items:center;justify-content:space-between;padding:8px 12px;background:linear-gradient(90deg,#dcfce7,#fef9c3);border-radius:8px;font-size:12px;border:1px solid #bbf7d0;margin-bottom:8px">
            <span style="font-weight:600;color:#166534">🎉 You're saving Rs. <%= savings_amount(@cart_items) %> on this order!</span>
          </div>
        <% end %>
        ...
        <%= for {pid, item} <- @cart_items do %>
          <div style="display:flex;align-items:center;gap:10px;padding:10px 0;border-bottom:1px solid #f3f4f6">
            ...
            <div style="flex:1;min-width:0">
              <div style="font-size:13px;font-weight:500;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:#111827"><%= item.name %></div>
              <div style="font-size:11px;color:#9ca3af;margin-top:2px">Rs. <%= Decimal.to_string(item.price) %> × <%= item.qty %></div>
            </div>
            <div style="display:flex;align-items:center;gap:5px">
              <button phx-click="decrement_cart" phx-value-product_id={pid}
                ...
                onmouseout="this.style.background='#f9fafb';this.style.color='inherit'">−</button>
              <span style="font-size:13px;font-weight:700;min-width:18px;text-align:center;color:#111827"><%= item.qty %></span>
            ...
            <button phx-click="remove_cart_item" phx-value-product_id={pid}
              ...
              onmouseout="this.style.color='#d1d5db';this.style.background='none'">🗑</button>
          </div>
        <% end %>
        ...
        <form phx-submit="apply_coupon" style="display:flex;gap:8px;margin:12px 0">
          ...
          <button type="submit"
            style="padding:8px 14px;background:#16a34a;color:#fff;border:none;border-radius:8px;font-size:12px;font-weight:700;cursor:pointer">Apply</button>
        </form>
        <%= if @coupon_error do %>
          <div style="font-size:11px;color:#ef4444;background:#fee2e2;padding:6px 10px;border-radius:6px;margin-bottom:8px"><%= @coupon_error %></div>
        <% end %>
        <%= if @coupon_discount do %>
          <div style="font-size:11px;color:#166534;background:#dcfce7;padding:6px 10px;border-radius:6px;margin-bottom:8px">🎉 <%= @coupon_discount.label %></div>
        <% end %>
        ...
        <div style="border-top:1px solid #e5e7eb;padding:10px 0">
          ...
          <%= if @coupon_discount do %>
            <div style="display:flex;justify-content:space-between;font-size:12px;padding:3px 0">
              ...
              <span style="color:#16a34a;font-weight:600">−Rs. <%= discount_amount(@cart_total, @coupon_discount) %></span>
            </div>
          <% end %>
      ...
      <div style="padding:14px 16px;border-top:1px solid #e5e7eb">
        <%= if @current_user do %>
          <button phx-click="close_cart"
            ...
            <span style="opacity:.85;font-size:13px">Rs. <%= grand_total(@cart_total, @coupon_discount) %></span>
          </button>
        <% else %>
