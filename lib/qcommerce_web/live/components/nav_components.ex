defmodule QcommerceWeb.Live.Components.NavComponents do
        ...
        <%= if length(@search_results) > 0 do %>
          <div style="position:absolute;top:calc(100% + 6px);left:0;right:0;background:#fff;border:1.5px solid var(--border);border-radius:12px;box-shadow:0 8px 30px rgba(0,0,0,.1);z-index:300;overflow:hidden">
            <%= for p <- @search_results do %>
              <div
                style="display:flex;align-items:center;justify-content:space-between;padding:10px 14px;cursor:pointer;transition:background .15s"
                phx-click="show_product"
  ...
  def location_modal(assigns) do
    ...
    <%= if @show_location_modal do %>
      <div
        style="position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:500;display:flex;align-items:flex-start;justify-content:center;padding-top:80px"
        phx-click="close_location_modal"
      >
        <div
          style="background:#fff;border-radius:16px;width:100%;max-width:440px;box-shadow:0 8px 32px rgba(0,0,0,.15);overflow:hidden"
          phx-click=""
        >
